...
for ( $i = 0; $i -lt $args.count; $i++ ) {
    ...
    if ($args[ $i ] -eq "-g") { 
        $samplesec = int(MinMaxSetting(1, 30, $args[ $i + 1 ]))
    }
    if ($args[ $i ] -eq "-j") { 
        $samplecount = int(MinMaxSetting(6, 24, $args[ $i + 1 ])) 
    }
    if ($args[ $i ] -eq "-k") { $fileheader = $args[ $i + 1 ] }
    if ($args[ $i ] -eq "-h") { 
        Quit("never mind.")
    }
}
...
do {
    ...
    if ($test_ping) {
        ...
        Write-Host "$computername ping" -NoNewline
        Write-Host "Eerror!" -ForegroundColor Red
    }
